...
#
# Warns once each time the session transcript crosses another threshold step,
# so a long-running session can't quietly grow into a multi-hundred-KB upload
...
size=$(stat -f%z "$transcript" 2>/dev/null || stat -c%s "$transcript" 2>/dev/null || printf '0')
...
msg="context-guard: this session's transcript is now ~${size_mb} MB. Every turn re-uploads the whole context, so each message is pushing roughly that much upstream — enough to saturate a slow uplink and add latency for everything else on the network. Run /compact to collapse it, or start a fresh session."
